...
export function measureQueryFetch(queryName: string, duration: number) {
  if (duration < 200) {
    console.log(`[Performance] ${queryName} fetch: ${duration.toFixed(2)}ms ✓`)
  } else if (duration < 500) {
    console.log(`[Performance] ${queryName} fetch: ${duration.toFixed(2)}ms ⚠️`)
  } else {
    console.warn(`[Performance] ${queryName} fetch: ${duration.toFixed(2)}ms ❌`)
  }
}
