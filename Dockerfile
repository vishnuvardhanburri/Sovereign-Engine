...
# Render dashboard-created services look for a Dockerfile at repo root.
# The actual app lives in /code, so copy that folder as the container root.
COPY code/ ./

RUN pnpm install --frozen-lockfile

# Build with safe placeholder values. Runtime configuration is supplied by Render env vars.
